...
//% block="BL MQTT publish$data to feed$feedName"
//% block.loc.fr="BL MQTT publier$data sur fil$feedName"
//% subcategory="Brilliant Labs Cloud"
//% group="MQTT"
//% weight=70   
//% blockGap=7
export function publishBLMQTT(feedName: string, data: number): void {
...
let BLAPIKey:string
// -------------- 3. Cloud ----------------
//% blockId=connectBLMQTT2
//% block="|BL MQTT connect with API Key%APIKey"
//% block.loc.fr="|BL MQTT connecter avec API clé%APIKey"
//% group="MQTT"
//% subcategory="Brilliant Labs Cloud"
//% weight=200   
//% blockGap=7
//% defl="bBoard_WiFi" 
export function connectBLMQTT(APIKey:string): void {
let currentBufferIndex = 0;

//              2           +       4             +         1            +          1           +         2        +        2              +    control.deviceSerialNumber().toString().length
//protocolNameLength.length + protocolName.length + protocolLevel.length + protocolFlags.length + keepAlive.length + clientIDLength.length + clientID.length 
let clientID = APIKey
let clientIDLength = APIKey.length
BLAPIKey = APIKey
...
bBoard_Control.UARTSendString("AT+CIPSTART=0,\"TCP\",\"cloud-alpha.brilliantlabs.ca\",1883,30\r\n",boardIDGlobal,clickIDGlobal)
