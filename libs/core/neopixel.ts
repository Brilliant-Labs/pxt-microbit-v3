...
namespace neopixel {
    ...
    /**
     * Sets NeoPixel Object for b.Board
     * @param Strip the Strip Object
     */
    //% block="bBoard"
    //% pin.defl=0
    //% blockSetVariable="bBoard_neoStrip"
    //% blockId=Strip
    //% weight=113
    export function createbBoardStrip(): Strip {
        return new Strip(BoardID.zero, ClickID.Zero, neoPin.none, 5, NeoPixelMode.RGB_RGB);
    }

    /**
     * Sets NeoPixel object for micro:bit
     *  @param Strip the Strip Object
     */
    //% block="pin $pin | with $numleds LEDs | and mode$mode"
    ...
    export function createmicroStrip(pin: DigitalPin, numleds: number, mode: NeoPixelMode): Strip {
        return new Strip(BoardID.zero, ClickID.Zero, parseInt(pin.toString()), numleds, mode);
    }
    ...
    //% blockId=Strip4
    //% weight=109
    export function createbBoadrAdvStrip(boardID: BoardID, clickID:ClickID, pin: neoPin, numleds: number, mode: NeoPixelMode): Strip {
    ...
    export class Strip extends bBoard_Control.peripheralSettings {
