...
​
    ...
    onButtonState(state: buttonPress, a: () => void): void {
      bBoard_Control.eventInit((state==buttonPress.Pressed)?bBoardEvents.CN_HIGH:bBoardEvents.CN_LOW, this.myBoardID, this.myClickID); //Tell the BLiX to set the Change notification interrupts (High or Low)
      bBoard_Control.pinEventSet(this.myBoardID,this.myClickID,clickIOPin.INT,(state==buttonPress.Pressed)?bBoardEvents.CN_HIGH:bBoardEvents.CN_LOW) //Tell the BLiX which pin you want to monitor for high or low
      control.onEvent(bBoard_Control.BLiX_INT_EVENT, bBoard_Control.getEventValue(this.myBoardID,this.myClickID,(state==buttonPress.Pressed)?bBoardEvents.CN_HIGH:bBoardEvents.CN_LOW), () => this.buttonEvent(state, a)); //Tell the DAL scheduler what function to call when the bBoard interrupt source is generated from this specific value
    }
​
    ...
    {    
      if(bBoard_Control.pinEventCheck(this.myBoardID,this.myClickID,clickIOPin.INT,(state==buttonPress.Pressed)?bBoardEvents.CN_HIGH:bBoardEvents.CN_LOW)) //Check the exact pin we are concerened with to see if it generated the interrupt
      {
