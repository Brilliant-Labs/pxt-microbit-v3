...
namespace Weather {
  ...
  export function createWeather(boardID:BoardID, clickID:ClickID): Weather {
      let handle = new Weather(boardID, clickID);
      handle.Power(PowerSettings.On);
    return handle
    }

    

    export class Weather extends bBoard_Control.I2CSettings{
        ...
        setreg(reg: number, dat: number): void {
            ...
            buf[0] = reg;
            buf[1] = dat;
            this.i2cWriteBuffer(this.Weather_I2C_Addr, buf);
        }
